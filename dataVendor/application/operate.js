...
async function create(contract, username, userID, amount){
    try{
        await contract.submitTransaction('CreateMyAsset', username, userID, amount);
...
async function update(contract, username, userID, amount){
    try{
        await contract.submitTransaction('UpdateMyAsset', username, userID, amount);
...
async function handle(contract){
    try{
        ...
        await contract.submitTransaction('SendResponse', resStr);
        return res.toString();
    }catch(err){
       return err;
    }
